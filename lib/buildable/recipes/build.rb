module Buildable::Recipe
  ...
  recipe :make_init_script do
    ...
    Buildable::Shell.do "foreman export initscript #{Buildable.upstart_folder} --user #{Buildable.config.app_user} --env production.env --app #{Buildable.config.project_name.downcase} --log /tmp -t #{Buildable.template_path 'master.erb'} -f Procfile -d #{Buildable.config.root_dir}"
